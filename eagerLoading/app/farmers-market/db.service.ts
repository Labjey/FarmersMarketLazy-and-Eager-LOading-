...
export class DbService {
  ...
  private addHardCodedData(){
    ...
    let obj4=new FarmProduction(4,'Natural Prairie 4');
    obj4.addProduce('barboronie');
    obj4.addProduce('PGHM');
  ...
  getData(id) {
    for (var idx in this.farmData){ 
      if(this.farmData[idx].getId()==id){ 
            return JSON.stringify(this.farmData[idx], undefined,  3cd lab);  
      }
    }
  }
  getAllData(){
